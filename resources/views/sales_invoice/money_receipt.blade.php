...
<body>
    ...
    <div id="scrollingContainer" class="col-xxs-9 content-container" style="overflow-x: hidden;width:100%;">
            ...
            @foreach ($sales_invoice_items as $key => $item)
                <tr class="page-break">
                    ...
                    {{-- <td>
                        ...
                        @else
                            <a href="{{ route('patient.sales_invoice.test_delete', $item->id) }}"
            ...
            @if ($sales_returnable != 0)
                @if (empty($sales_return[0]) || $sales_return[0]->amount != $sales_returnable)
                    ...
                    <tr class="border-none">
                        ...
                        {{-- <td> <a href="{{ route('patient.sales_invoice.return_initiate', $sales_invoice->id) }}"
            ...
            @if ($is_ready)
                @if ($due_amount > 0)
                    <tr class="border-none">
                        ...
                        {{-- <td>
                            <a href="{{ route('patient.sales_invoice.pay_now', $sales_invoice->id) }}"
    ...
    <htmlpagefooter name="page-footer">
        ...
        <div style="font-size: 10px; ">Report delivery time : 8:00 am to 8:00 pm. Authority will not be responsible if Report is not collected within one month </div>
        <div  style="font-size: 10px; ">* Online test Report will be available after 8pm, on Report date for Tests with no due, <br>    &nbsp;&nbsp;&nbsp;To avail online Test Report, go to www.lifefertilitycentre.com using your UserId/password: {{$sales_invoice->patient->code }}</div>
        <div  style="font-size: 10px; ">   *Terms and Condition Applicable</div>
        <div  style="font-size: 10px; ">This is a computer generated document, does not require any signature </div>

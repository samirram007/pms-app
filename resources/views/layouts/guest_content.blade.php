<div class="wrapper">
    <div class="steady1  bg-cover d-flex align-items-center">
        <div class="container">
            <div class="row">
                <div class=" col-md-6">
                    <h1 class="fw-bold fs-3 my-3 text-black">"One rarely falls in love without being as much attracted
                        to what is interestingly wrong with someone as what is objectively healthy."</h1>

                </div>
                <div class="col-md-6 ">
                    <div class="rounded ">
                        <div class="card login_form">
                            <div class="card-body">
                                <h3 class="card-title fs-2 text-center  fw-bold">Login here...</h3>
                                <form action="" method="post">
                                    @csrf
                                    <div class="form-floating mb-3">
                                        <input type="email" class="form-control rounded-2" id="floatingInput"
                                            placeholder="name@example.com">
                                        <label for="floatingInput">Email ess</label>
                                    </div>
                                    <div class="form-floating mb-3">
                                        <input type="password" class="form-control rounded-2" id="floatingPassword"
                                            placeholder="Password">
                                        <label for="floatingPassword">Password</label>
                                    </div>
                                    <button class="btn btn-dark btn-lg rounded w-100">LogIn</button>
                                </form>
                                <div class="mt-2">
                                    <a href="" class="fw-bold fs-6">Forgot Password?</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<section class="services">
    <div class="container">
        <h2 class="text-center fw-bold fs-2" style="font-size: 32px; color:rgb(121, 81, 132)">Services</h2>
        <p class="text-center">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Omnis at fugiat laudantium ame
        </p>
        <div class="row gy-4 mt-5">
            <div class="col-md-6 col-lg-4" style="flex: 0 0 auto; width:33.33%;">
                <div class="service-item position-relative">
                    <div class="icon">
                        <i class="bi bi-activity"></i>
                    </div>
                    <h3>lorem Test</h3>
                    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sit, quae suscipit mollitia velit
                        voluptatibus officiis dolor est facilis?</p>
                    <a href="" class="readmore stretched-link fw-bold fs-5 text-success">Book-></a>
                </div>

            </div>
            <div class="col-md-6 col-lg-4" style="flex: 0 0 auto; width:33.33%;">
                <div class="service-item position-relative">
                    <div class="icon">
                        <i class="bi bi-activity"></i>
                    </div>
                    <h3>lorem Test</h3>
                    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sit, quae suscipit mollitia velit
                        voluptatibus officiis dolor est facilis? </p>
                    <a href="" class="readmore stretched-link fw-bold fs-5 text-success">Book-></a>
                </div>

            </div>
            <div class="col-md-6 col-lg-4" style="flex: 0 0 auto; width:33.33%;">
                <div class="service-item position-relative">
                    <div class="icon">
                        <i class="bi bi-activity"></i>
                    </div>
                    <h3>lorem Test</h3>
                    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sit, quae suscipit mollitia velit
                        voluptatibus officiis dolor est facilis? </p>
                    <a href="" class="readmore stretched-link fw-bold fs-5 text-success">Book-></a>
                </div>

            </div>
            <div class="col-md-6 col-lg-4" style="flex: 0 0 auto; width:33.33%;">
                <div class="service-item position-relative">
                    <div class="icon">
                        <i class="bi bi-activity"></i>
                    </div>
                    <h3>lorem Test</h3>
                    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sit, quae suscipit mollitia velit
                        voluptatibus officiis dolor est facilis? </p>
                    <a href="" class="readmore stretched-link fw-bold fs-5 text-success">Book-></a>
                </div>

            </div>
            <div class="col-md-6 col-lg-4" style="flex: 0 0 auto; width:33.33%;">
                <div class="service-item position-relative">
                    <div class="icon">
                        <i class="bi bi-activity"></i>
                    </div>
                    <h3>lorem Test</h3>
                    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sit, quae suscipit mollitia velit
                        voluptatibus officiis dolor est facilis? </p>
                    <a href="" class="readmore stretched-link fw-bold fs-5 text-success">Book-></a>
                </div>

            </div>
            <div class="col-md-6 col-lg-4" style="flex: 0 0 auto; width:33.33%;">
                <div class="service-item position-relative">
                    <div class="icon">
                        <i class="bi bi-activity"></i>
                    </div>
                    <h3>lorem Test</h3>
                    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sit, quae suscipit mollitia velit
                        voluptatibus officiis dolor est facilis? </p>
                    <a href="" class="readmore stretched-link fw-bold fs-5 text-success">Book-></a>
                </div>

            </div>
        </div>
    </div>
</section>
<div class="contact mt-4 mb-4 py-3 container" style="  box-shadow: -1px 2px 25px 12px rgb(0, 0, 0, 20%);">
    <div class="text-center mt-4">
        <h1 class="fw-bold mb-2 pb-2 position-relative" style="font-size: 32px; color:rgb(121, 81, 132)">Contact</h1>

    </div>
    <div class="row gx-lg-0 gy-4">
        <div class="col-lg-4 col-md-4 col-sm-4">
            <div class="info-container d-flex flex-column align-items-center justify-content-center">
                <div class="info-item d-flex">
                    <i class="bi bi-telephone flex-shrink-0"></i>
                    <div>
                        <h4>Call:</h4>
                        <p>9999999999</p>
                    </div>
                </div>
                <div class="info-item d-flex">
                    <i class="bi bi-telephone flex-shrink-0"></i>
                    <div>
                        <h4>Email:</h4>
                        <p>patho123@gmail.com</p>
                    </div>
                </div>
                <div class="info-item d-flex">
                    <i class="bi bi-telephone flex-shrink-0"></i>
                    <div>
                        <h4>Open Hours:</h4>
                        <p>Mon-Sun: 10Am - 23PM</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-8">
            <div class="px-3">
                <h1 class="text-center fw-bold fs-2">Book an Appoinment</h1>
                <p class="mt-2">Lorem ipsum dolor, sit amet consectetur adipisicing elit. Unde, quam odit? Debitis
                    accusantium tempora, consequuntur laboriosam .</p>
                <form action="" method="post" role="form" class="php-email-form mt-3">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 mt-2 form-group">
                            <input type="text" name="name" id="" class="form-control" placeholder="Your Name">
                        </div>
                        <div class="col-md-6 mt-2 form-group">
                            <input type="email" name="email" id="" class="form-control" placeholder="Your Email">
                        </div>
                        <div class="col-md-6 mt-3 form-group">
                            <input type="number" name="contact_no" id="" class="form-control"
                                placeholder="Your Phone Number">
                        </div>
                        <div class="col-md-6 mt-3 form-group">
                            <select class="form-select" aria-label="Default select example">
                                <option selected>Select Test Type</option>
                                <option value="1">One</option>
                                <option value="2">Two</option>
                                <option value="3">Three</option>
                            </select>
                        </div>
                        <div class="col-md-6 mt-3 form-group">
                            <label for="" class="text-black fw-bold">Appointment Date</label>
                            <input type="date" name="contact_no" id="" class="form-control"
                                placeholder="Your Phone Number">
                        </div>
                        <div class="col-md-6 mt-3 form-group">
                            <label for="" class="fw-bold text-black">Appoinment Time</label>
                            <input type="time" name="contact_no" id="" class="form-control"
                                placeholder="Your Phone Number">
                        </div>
                    </div>

                    <div class="mt-4 align-items-center d-flex justify-content-center">
                        <button class="btn btn-success w-50">Book</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
